...
$select_dis = "SELECT  district FROM schools GROUP BY district ORDER BY district";
$dis_query = $connection->query($select_dis);
$result_dis = $dis_query->fetchAll(PDO::FETCH_ASSOC);



$range = isset($_GET["range"]) ? $_GET["range"] : 05;
$district = isset($_GET["district"]) ? $_GET["district"] : "";
$search = isset($_GET["search"]) ? $_GET["search"] : "";

if (isset($_GET['search'])) {
    $searchval = $_GET['search'];
    $recordCountSql = "SELECT count(*) as Count FROM schools WHERE s_name  LIKE '%$searchval%' || district  LIKE '%$searchval%' || board  LIKE '%$searchval%'";
    $result = $connection->query($recordCountSql);
    ...
    $select = "SELECT * FROM schools WHERE s_name  LIKE '%$searchval%' OR district  LIKE '%$searchval%' OR board  LIKE '%$searchval%' LIMIT " . (($pageNo - 1) * $range) . ",$range";
    $query = $connection->query($select);
    $result = $query->fetchAll(PDO::FETCH_ASSOC);
} elseif (isset($_GET['district'])) {
    $districtval = $_GET["district"];
    $recordCountSql = "SELECT count(*) as Count FROM schools WHERE district = '$districtval'";
    $result = $connection->query($recordCountSql);
    $countRecord = $result->fetchAll(PDO::FETCH_ASSOC)[0]["Count"];
    $pageCount = $countRecord % $range == 0 ? ($countRecord / $range) : intval($countRecord / $range) + 1;
    if (!isset($_GET["page"])) {
    ...
    $select = "SELECT * FROM schools WHERE district = '$districtval' ORDER BY index_no LIMIT " . (($pageNo - 1) * $range) . ",$range";
    $query = $connection->query($select);
    $result = $query->fetchAll(PDO::FETCH_ASSOC);
} else {
    $recordCountSql = "SELECT count(*) as Count FROM schools";
    $result = $connection->query($recordCountSql);
    $countRecord = $result->fetchAll(PDO::FETCH_ASSOC)[0]["Count"];
    $pageCount = $countRecord % $range == 0 ? ($countRecord / $range) : intval($countRecord / $range) + 1;
    if (!isset($_GET["page"])) {
    ...
    $select = "SELECT * FROM schools  ORDER BY index_no LIMIT " . (($pageNo - 1) * $range) . ",$range";
    $query = $connection->query($select);
    $result = $query->fetchAll(PDO::FETCH_ASSOC);
}
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <ul class="insight_core_breadcrumb">
                                            ...
                                            <li class="level-2 sub tail current"><span>Schools</span></li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="container">
                <div class="row d-flex">
                    ...
                    <div class="col-lg-2 col-md-4 col-sm-6 col-xs-12">
                        <label for="exampleFormControlInput1" class="form-label">Rows Per Page</label>
                        <select name="range" class="border border-success" onchange="changerange()" id="range">
                            <option value="05" <?= $range == 05 ? "selected" : '' ?>>5</option>
                    ...
                    <div class="col-lg-2 col-md-4 col-sm-6 col-xs-12">
                        ...
                        <select name="sort" class="border border-success" onchange="changeSort()" id="sort">
                            <option value="Index No." selected>Index No.</option>
                            <option value="District">District</option>
                        </select>
                    </div>
                    <div id="dis" class="col-lg-2 col-md-4 col-sm-6 col-xs-12">
                    </div>
                    ...
                    <div class=" col-lg-2 col-md-4 col-sm-6 col-xs-12">
                        ...
                        <input class="form-control me-2 border border-success" type="text" name="search" id="search" onchange="search()" placeholder="Search" aria-label="Search">
                    </div>
                                                                                    ...
                                                                                    <div class="grid-item post-item post-5168 post type-post status-publish format-video has-post-thumbnail hentry category-business tag-finance post_format-post-format-video pmpro-has-access">
                                                                                        <div class="post-wrapper maxcoach-box">
                                                                                            ...
                                                                                            <div class="post-caption">
                                                                                                ...
                                                                                                <h3 class="post-title">
                                                                                                    <a href="<?= $ans['website'] ?>" target="_blank" class="scname" title="<?= $ans['s_name'] ?>"><?= $ans['s_name'] ?></a>
                                                                                                </h3>
                                                                                                <div class="post-meta">
                                                                                                    <div class="inner">
                                                                                                        <div class="post-date">
                                                                                                            <span class="meta-icon"><i class="far fa-light fa-user"></i></span>
                                                                                                            <?= $ans['index_no'] ?>
                                                                                                        </div>

                                                                                                        <div class="post-view">
                                                                                                            <span class="meta-icon">
                                                                                                                <i class="far fa-light fa-route-interstate"></i>
                                                                                                            </span>
                                                                                                            <?= $ans['district'] ?>
                                                                                                        </div>
                                                                                                ...
                                                                                                <div class="post-meta">
                                                                                                    <div class="inner">
                                                                                                        ...
                                                                                                        <div class="post-email">
                                                                                                            <span class="meta-icon">
                                                                                                                <i class="far fa-light fa-graduation-cap"></i>
                                                                                                            </span>
                                                                                                            <?= $ans['board'] ?>
                                                                                                ...
                                                                                                <div class="post-excerpt">
                                                                                                    <p title="<?= $ans['description'] ?>"><?= $ans['description'] ?></p>
                                                                                                </div>
                                                                                                <?php if (isset($_SESSION['user'])) { ?>
                                                                                                    <?php if ($_SESSION["user_type"] == 'Government Panel' || $_SESSION["user_type"] == 'School Panel') { ?>
                                                                                                        <?php if ($_SESSION["user"] == $ans['s_name']) { ?>
                                                                                                            <div class="post-footer">
                                                                                                                <div class="post-share style-01">
                                                                                                                    ...
                                                                                                                    <div class="share-media">
                                                                                                                        ...
                                                                                                                        <div class="share-list" style="display: flex;">
                                                                                                                            <!-- <form action="./add_school_student.php" method="post"> -->
                                                                                                                            <a class="hint--bounce hint--top hint--primary facebook" target="_self" aria-label="Add Student" href="../includes/add_school_student.php">
                                                                                                                                <button type="submit" name="add_stud" style="background-color:orange; border-color:orange;border-radius:50%; padding:0px 20px 20px;"><i class="fab fa fa-plus" id="addstudent"></i></button>
                                                                                                                            </a>
                                                                                                                            <?php
                                                                                                                            // session_start();

                                                                                                                            $_SESSION["school_id"] = $ans["id"];
                                                                                                                            ?>
                                                                                                                            <!-- </form> -->
                                                                                                                            ...
                                                                                                                            <a class="hint--bounce hint--top hint--primary twitter" target="_self" aria-label="View Student" href="../includes/all_student.php">
                                                                                                                                <button type="submit" name="view" style="background-color:grey; border-color:grey;border-radius:50%; padding:2px 16px 20px;"><i class="fab fa fa-eye"></i></button>
                                                                                                                            </a>
                                                                                                                            <!-- </form> -->
                                                                                                                            <form action="./update_school.php" method="post">
                                                                                                                                <a class="hint--bounce hint--top hint--primary linkedin" target="_self" aria-label="Update School" href="../includes/update_school.php">
                                                                                                                                    <button type="submit" value="<?= $ans["id"] ?>" name="update_id" style="border-radius:50%; padding:0px 20px 20px;"><i class="fab fa fa-arrow-up"></i></button>
                                                                                                                                </a>
                                                                                                                            </form>
                                                                                                                        </div>
                                                                                                                    </div>
                                                                                        ...
                                                                                        <ul class="page-pagination">
                                                                                            ...
                                                                                            for ($i = 1; $i <= $pageCount; $i++) {
                                                                                                if (!isset($_GET["page"]))
                                                                                                    $_GET["page"] = 1;
                                                                                            ?>
                                                                                                <li class="page-item <?= $_GET["page"] == $i ? "active" : "" ?>"><a class="page-link" href="all_schools.php?page=<?= $i ?>&range=<?= $range ?>"><?= $i ?></a></li>
                                                                                            <?php
                                                                                            }
                                                                                            ?>
                                                                                        </ul>
 ...
 -->
    ...
    <script>
        $('#allstudent').click(function(e) {
            ...
            $.ajax({
                url: '/ssip/assets/codes/student_check.php',
                type: 'POST',
                ...
                success: function(data) {
                    ...
                    if (data == 1) {
                        ...
                        // window.location.href = './all_student.php?name=<?= $school ?>';
                        // window.location.href = "stock.php?range=" + range;
                    } else {
                        // window.location.href = './all_student.php?name=<?= $college ?>';
                    }
                }
            });
        });
        ...
        function changerange() {
            ...
            window.location.href = "all_schools.php?range=" + range;
        }
        ...
        function changeSort() {
            ...
            if (sort == "District") {
                ...
                <select name="dis" class="" onchange="changedis()" id="district">
                    <?php foreach ($result_dis as $record) { ?>
                        <option value="<?= $record["district"] ?>" <?= $record["district"] == $record["district"] ? "selected" : '' ?>><?= $record["district"] ?></option>
                    <?php } ?>
                    </select>
                </div>`);
            } else {
        ...
        function changedis() {
            ...
            window.location.href = "all_schools.php?district=" + district;
        }

        function search() {
            var search = document.getElementById('search').value;
            window.location.href = "all_schools.php?search=" + search;
        }
    </script>
</body>
